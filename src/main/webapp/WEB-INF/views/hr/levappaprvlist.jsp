	...
	<div class="card-body">
		<div class="d-flex justify-content-between align-items-center mb-4">
			...
			<div class="d-flex justify-content-end" style="width: 70%;">
				<form action="<c:url value='/hr/hrapplication?levPageNo=1'/>"
					method="POST">
					<div class="d-flex justify-content-end">
						...
						<div id="datepicker-leave"
							...
							<input type="text"
								...
								name="levStartDate" style="border: 0px; text-align: center;">
							<span
				...
				<ul class="pagination pb-0 mb-0">
					...
					<c:if test="${levPager.groupNo > 1}">
						<li class="page-item disabled"><a class="page-link"
							href="<c:url value='/hr/hrapplication?levPageNo=${levPager.startPageNo-5}&levStartDate=${levStartDate}&levEndDate=${levEndDate}'/>">이전</a>
						</li>
					</c:if>
					<c:forEach var="i" begin="${levPager.startPageNo}"
						end="${levPager.endPageNo}" step="1">
						<!-- 선택할 페이지 -->
						<c:if test="${levPager.pageNo != i}">
							<li class="page-item"><a class="page-link"
								href="<c:url value='/hr/hrapplication?levPageNo=${i}&levStartDate=${levStartDate}&levEndDate=${levEndDate}'/>">${i}</a>
							</li>
						</c:if>
						<!-- 현재페이지 -->
						<c:if test="${levPager.pageNo == i}">
					...
					<c:if test="${levPager.groupNo < levPager.totalGroupNo}">
						<li class="page-item"><a class="page-link"
							href="<c:url value='/hr/hrapplication?levPageNo=${levPager.startPageNo+5}&levStartDate=${levStartDate}&levEndDate=${levEndDate}'/>">다음</a></li>
					</c:if>
				</ul>
			</div>
		</c:if>
