								...
								<div class="table-responsive">
									...
									<div class="mt-2" >
										...
										<c:if test="${pager.totalRows > 0}">
											<!-- 처음 -->
											<li class="page-item"><a class="page-link" href="<c:url value='/approval/returneddocument/1'/>">처음</a></li>
											
											<!-- 이전 -->
											<c:if test = "${pager.groupNo > 1}">
											<li class="page-item"><a class="page-link" href="<c:url value='/approval/returneddocument/${pager.startPageNo-1}'/>">이전</a></li>
											</c:if>
											
											<!-- 페이지그룹 -->
											<c:forEach var="i" begin="${pager.startPageNo}" end ="${pager.endPageNo}">
											<c:if test="${pager.pageNo != i}">
											<li class="page-item" ><a class="page-link" href="<c:url value='/approval/returneddocument/${i}'/>">${i}</a></li>
											</c:if>
											<c:if test="${pager.pageNo == i}">
											<li class="page-item active" ><a class="page-link" href="<c:url value='/approval/returneddocument/${i}'/>">${i}</a></li>
											</c:if>
											</c:forEach>
											
											<!-- 다음 -->
											<c:if test = "${pager.groupNo < pager.totalGroupNo }">
											<li class="page-item"><a class="page-link" href="<c:url value='/approval/returneddocument/${pager.endPageNo+1}'/>">다음</a></li>
											</c:if>
											
											<!-- 마지막 -->
											<li class="page-item"><a class="page-link" href="<c:url value='/approval/returneddocument/${pager.totalPageNo}'/>">마지막</a></li>
										</c:if>
										</ul>
									</div>
								</div>
