	        	...
	        	<div class="row">
	        		<div class="col-md-12 grid-margin">
	        			<div class="card gird-margin shadow-2">
		        			<div class="card-body">
		        				<p class="card-title mb-3">완결문서함</p>
		        				<div class="card gird-margin shadow-2 my-3" style="background-color: #e7e7ff;">
			        			<div class="card-body">
		        					<div class="row py-0" style="padding: 1.25rem;">
		        						<div class="d-flex">
	        								<form id="searchForm" class="d-flex" action="<c:url value="/approval/completeddocument/search"></c:url>" method="get">
	        								<div class="input-group w-75" style="border-radius: 18px; background-color: white;">
												...
												<div id="searchBar" class="align-self-center">
													...
													<div id="완결날짜" class="badge badge-warning font-weight-bold">
														<input type="hidden" name="docCompleteStartDate" value="${searchQuery.docCompleteStartDate}"></input>
														<input type="hidden" name="docCompleteEndDate" value="${searchQuery.docCompleteEndDate}"></input>
								...
								<div class="table-responsive">
									...
									<div class="mt-5">
										...
										<c:if test="${pager.totalRows > 0}">
											<!-- 처음 -->
											<li class="page-item"><a class="page-link" href="<c:url value='/approval/completeddocument/1'/>">처음</a></li>
											
											<!-- 이전 -->
											<c:if test = "${pager.groupNo > 1}">
											<li class="page-item"><a class="page-link" href="<c:url value='/approval/completeddocument/${pager.startPageNo-1}'/>">이전</a></li>
											</c:if>
											
											<!-- 페이지그룹 -->
											<c:forEach var="i" begin="${pager.startPageNo}" end ="${pager.endPageNo}">
											<c:if test="${pager.pageNo != i}">
											<li class="page-item" ><a class="page-link" href="<c:url value='/approval/completeddocument/${i}'/>">${i}</a></li>
											</c:if>
											<c:if test="${pager.pageNo == i}">
											<li class="page-item active" ><a class="page-link" href="<c:url value='/approval/completeddocument/${i}'/>">${i}</a></li>
											</c:if>
											</c:forEach>
											
											<!-- 다음 -->
											<c:if test = "${pager.groupNo < pager.totalGroupNo }">
											<li class="page-item"><a class="page-link" href="<c:url value='/approval/completeddocument/${pager.endPageNo+1}'/>">다음</a></li>
											</c:if>
											
											<!-- 마지막 -->
											<li class="page-item"><a class="page-link" href="<c:url value='/approval/completeddocument/${pager.totalPageNo}'/>">마지막</a></li>
										</c:if>
										</ul>
									</div>
								</div>
