				...
				<tbody>
						...
						<c:forEach items="${sendmail}" var="send">
							...
							<c:if test="${send.tbName =='send' }">
								...
								<td class="form-inline pr-0 p1-2">
									...
									<button onclick="star(${send.sendMailId})"><i class="h3 mdi mdi-star text-warning"></i></button>
								...
								<c:if test="${send.tbName =='send' }">
									<td onclick="location.href='<c:url value="/mail/detailmail/import/${send.sendMailId}/send"/>'" style="font-weight:bold;">${send.sendMailTitle}</td>
								</c:if>
								<c:if test="${send.tbName =='received' }">
									<td onclick="location.href='<c:url value="/mail/detailmail/import/${send.sendMailId}/received"/>'" style="font-weight:bold;">${send.sendMailTitle}</td>
								</c:if>
					...
					<c:if test="${empty sendmail}">
						<tr>
							...
							<td></td>
							<td></td>
							<td  class="p-5" style="font-size:20px;">중요 메일함이 비었습니다.</td>
							<td></td>
						</tr>
					</c:if>
				</tbody>
